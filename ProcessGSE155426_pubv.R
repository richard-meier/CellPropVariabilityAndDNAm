...
download.file(
  url = "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE155nnn/GSE155426/suppl/GSE155426_processed_beta.txt.gz", 
...
IDS = sapply(
  ...
  FUN = function(x){return(
    paste0("X",str_replace_all(as.character(x),"-","."))
  )}
)
names(IDS) = NULL
...
covariates$blast_tbi=as.character(covariates$"blast_tbi:ch1")
covariates$breaching=as.character(covariates$"breaching history:ch1")
covariates$session=as.character(covariates$"session:ch1")
# covariates$gender = as.character(covariates$"sex:ch1") # all of them are male
covariates$tbi=as.character(covariates$"tbi:ch1")
covariates$time=as.character(covariates$"time:ch1")
...
estimateSD = function(Y,meta){
  return( summary(lm(Y~age+blast_tbi+breaching+tbi+session+time,data=meta))$sigma )
}
