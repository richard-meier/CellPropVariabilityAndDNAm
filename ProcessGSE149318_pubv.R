...
download.file(
	url = "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE149nnn/GSE149318/suppl/GSE149318_beta_values.txt.gz", 
...
# move IDs from a separate column into the row names
...
# construct new IDs and update the row names
#  -> this step is likely very different in every dataset
IDS = paste0("X",covariates$sentrix_id,".",covariates$sentrix_id,"_",covariates$sentrix_position)
row.names(covariates) = IDS

# if "covariates" contains samples that are not whole blood exclude them here !
#    -> excluding smaples from the "covariates" table is sufficient (code below handles the rest)
covariates = covariates[covariates$"tissue:ch1" == "Blood",]
...
covariates$bmi = as.numeric(covariates$"bmi:ch1")
#covariates$sex = as.character(covariates$"Sex:ch1") # all of them are male
covariates$cohort = as.character(covariates$"cohort:ch1")
boxplot(bmi~cohort,data=covariates)
...
estimateSD = function(Y,meta){
  return( summary(lm(Y~age+bmi+cohort,data=meta))$sigma )
}
