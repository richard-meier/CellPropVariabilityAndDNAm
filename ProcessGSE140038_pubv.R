...
betas = read.csv(
  paste0(dataset_directory,"/",gse.id,"_beta_values.csv")
)
...
covariates$time.point = as.numeric(covariates$"time point:ch1")
covariates$chemotherapy.any = as.numeric(covariates$"chemotherapy any:ch1")
...
estimateSD = function(Y,meta){
  return( summary(lm(Y ~ age + time.point + chemotherapy.any,data=meta))$sigma )
}
